...
// Set up input values for first round
    L0 = pbData[0] << 24 | pbData[1] << 16 | pbData[2] << 8 | pbData[3];
    L1 = pbData[4] << 24 | pbData[5] << 16 | pbData[6] << 8 | pbData[7];
    R0 = pbData[8] << 24 | pbData[9] << 16 | pbData[10] << 8 | pbData[11];
    R1 = pbData[12] << 24 | pbData[13] << 16 | pbData[14] << 8 | pbData[15];
...
// Copying output values from last round to pbData
    pbData[0] = ((R0 >> 24) & 0xff); pbData[1] = ((R0 >> 16) & 0xff); pbData[2] = ((R0 >> 8) & 0xff); pbData[3] = (R0 & 0xff);
    pbData[4] = ((R1 >> 24) & 0xff); pbData[5] = ((R1 >> 16) & 0xff); pbData[6] = ((R1 >> 8) & 0xff); pbData[7] = (R1 & 0xff);
    pbData[8] = ((L0 >> 24) & 0xff); pbData[9] = ((L0 >> 16) & 0xff); pbData[10] = ((L0 >> 8) & 0xff); pbData[11] = (L0 & 0xff);
    pbData[12] = ((L1 >> 24) & 0xff); pbData[13] = ((L1 >> 16) & 0xff); pbData[14] = ((L1 >> 8) & 0xff); pbData[15] = (L1 & 0xff);
}
...
#define RoundKeyUpdate0(K, A, B, C, D, KC) {	\
    ...
    (K)[1] = SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^  \
             SS2[GetB2(T1)] ^ SS3[GetB3(T1)];   \
    T0 = A;                                     \
    A = (A>>8&0x00ffffff) ^ (B<<24&0xff000000); \
    B = (B>>8&0x00ffffff) ^ (T0<<24&0xff000000);\
}
...
#define RoundKeyUpdate1(K, A, B, C, D, KC) {	\
    ...
    (K)[1] = SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^  \
             SS2[GetB2(T1)] ^ SS3[GetB3(T1)];   \
    T0 = C;                                     \
    C = (C<<8&0xffffff00) ^ (D>>24&0x000000ff); \
    D = (D<<8&0xffffff00) ^ (T0>>24&0x000000ff);\
}
...
// Set up input values for Key Schedule
    A = pbUserKey[0] << 24 | pbUserKey[1] << 16 | pbUserKey[2] << 8 | pbUserKey[3];
    B = pbUserKey[4] << 24 | pbUserKey[5] << 16 | pbUserKey[6] << 8 | pbUserKey[7];
    C = pbUserKey[8] << 24 | pbUserKey[9] << 16 | pbUserKey[10] << 8 | pbUserKey[11];
    D = pbUserKey[12] << 24 | pbUserKey[13] << 16 | pbUserKey[14] << 8 | pbUserKey[15];
